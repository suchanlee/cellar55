...
const splitReg: RegExp = /,|with|aka|and|balance|is|\/|\&/gi;
...
export class VarietalFilter extends React.PureComponent<Props, State> {
  ...
  public render() {
    let sortedVarietalEntries: VarietalEntry[] = reverse(
      sortBy(values(this.getVarietalMap()), "count")
    );
    if (!this.state.isShowAll) {
      sortedVarietalEntries = sortedVarietalEntries.slice(
        0,
        CLOSED_VARIETAL_COUNT
      );
    }
    return (
      <BaseFilter filterKey="Varietal">
        <div className="varietal-filter-values-container">
          {map(sortedVarietalEntries, entry => {
            ...
            return (
              <span className="item varietal-filter-item" key={entry.name}>
                <CheckboxInput
                  checked={checked}
